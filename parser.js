	...
	message: function (message, room) {
		...
		switch (spl[1]) {
			...
			case 'J': case 'j':
				...
				//if (toId(user.id)=="shadebones" || toId(user.id)=="4mat" && room.id=="boxbattles") this.say(room, "/sendhtmlpage " + toId(user.id) + ", Error, You may be receiving this message due to not having a registered character on Box Battles."); // Removed via staff request
